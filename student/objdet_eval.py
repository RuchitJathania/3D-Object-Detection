...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            #corners = tools.compute_box_corners(x,y,w,l,yaw)
            corners_gt = tools.compute_box_corners(label.box.center_x, label.box.center_y, label.box.width, label.box.length, label.box.heading)
            #corners = [fl,rl,rr,fr]
            ## step 2 : loop over all detected objects
            for item in detections:
                _,x,y,z,h,w,l,yaw = [item[i] for i in range(len(item))]
                #print(x,y,w,l,yaw)
                ## step 3 : extract the four corners of the current detection
                corners_p =  tools.compute_box_corners(item[1],item[2],item[5],item[6],item[7])
                #print(corners_gt)
                #print(corners_p)
                ...
                polygon1 = Polygon(corners_gt)
                polygon2 = Polygon(corners_p)
    ...
    false_negatives = 0
    false_negatives = sum(labels_valid) - len(ious)
    ## step 3 : compute the number of false positives
    false_positives = 0
    if len(detections)>=sum(labels_valid):
        false_positives = len(detections) - sum(labels_valid)
    #######
    ####### ID_S4_EX2 END #######     
