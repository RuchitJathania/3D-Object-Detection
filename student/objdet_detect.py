...
def create_model(configs):
    ...
    if (configs.arch == 'darknet') and (configs.cfgfile is not None):
        ...
        model = fpn_resnet.get_pose_net(18,configs.heads,configs.head_conv,configs.pretrained_filename)
        # num_layers, heads, head_conv, imagenet_pretrained
        #######
        ####### ID_S3_EX1-4 END #######     
...
def detect_objects(input_bev_maps, model, configs):
    ...
    with torch.no_grad():  
        ...
        if 'darknet' in configs.arch:
            ...
            #hm_cen, cen_offset, direction, z_coor, dim, K=40
            detections1 = decode(outputs['hm_cen'],outputs['cen_offset'],outputs['direction'],outputs['z_coor'],outputs['dim'],K=40)
            out_post = post_processing(detections1.cpu().numpy().astype(np.float32), configs)
            print(out_post)
            for key, value in out_post[0].items():
                print(value)
                if value is None:
                    continue
                for item in value:
    ...
    for item in detections:
        ...
        #print(configs.bev_width,configs.bev_height)
        yn = -(((configs.bev_height - x)/configs.bev_height)*50) + 25
        xn = (y/configs.bev_width)*50
        w = ((w/configs.bev_width) * 50)
        l = ((l/configs.bev_width) * 50)
            ## step 4 : append the current object to the 'objects' array
        objects.append([ _, xn, yn, z, h, w, l, yaw])
        #print([ _, xn, yn, z, h, w, l, yaw])
    #######
    ####### ID_S3_EX2 START #######   
