...
def show_range_image(frame, lidar_name):
    ...
    r1 = [rimg for rimg in frame.lasers if rimg.name == lidar_name][0]
    ri=[]
    if len(r1.ri_return1.range_image_compressed) > 0: # use first response
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    discre = (configs.lim_x[1]-configs.lim_x[0])/configs.bev_height
    ...
    bev_int = np.zeros((configs.bev_height,configs.bev_width))

    # step 2 : re-arrange elements in lidar_pcl_cpy by sorting first by x, then y, then -z (use numpy.lexsort)
    sort1 = np.lexsort((-lidar_pcl_copy[:,:3][:,2],lidar_pcl_copy[:,:3][:,1],lidar_pcl_copy[:,:3][:,0]))

    ## step 3 : extract all points with identical x and y such that only the top-most z-coordinate is kept (use numpy.unique)
    ##          also, store the number of points per x,y-cell in a variable named "counts" for use in the next task
    top_pcl = lidar_pcl_copy[sort1]
    topUniq = np.unique(top_pcl[:,:2], axis=0, return_index=True,return_counts=True)
    counts = topUniq[2]
    top_pcl = top_pcl[topUniq[1]]

    ## My own intensity sort/unique vals for intensity map, such that for the same x and y points, the one with greatest intensity value is kept
    sort2 = np.lexsort((-lidar_pcl_copy[:,(0,1,3)][:,2],lidar_pcl_copy[:,(0,1,3)][:,1],lidar_pcl_copy[:,(0,1,3)][:,0]))
    int_pcl = lidar_pcl_copy[sort2]
    intUniq = np.unique(int_pcl[:,:2], axis=0, return_index=True)
    int_pcl = int_pcl[intUniq[1]]
    idx_lim = int_pcl[:,3] > 1.0
    int_pcl[idx_lim,3] = 1.0
    ...
    ## step 1 : create a numpy array filled with zeros which has the same dimensions as the BEV map
    bev_height = np.zeros((configs.bev_height,configs.bev_width))
    ## step 2 : assign the height value of each unique entry in lidar_top_pcl to the height map 
    ##          make sure that each entry is normalized on the difference between the upper and lower height defined in the config file
    ##          use the lidar_pcl_top data structure from the previous task to access the pixels of the height_map
    bev_height[np.int_(top_pcl[:,0]),np.int_(top_pcl[:,1])] = top_pcl[:,2]/float(np.abs(configs.lim_z[1]-configs.lim_z[0]))
